...
We used automatic procedures as followed to find a ‘best subset’of the full model. 
```{r}
...
bf_data_adj= bf_data %>%
  select(-knee,-bicep,-ankle) %>%
  mutate(height_neck=height*neck)

glmnet(as.matrix([2:6]), swiss$Fertility, lambda = 1, alpha = 0.5)
```